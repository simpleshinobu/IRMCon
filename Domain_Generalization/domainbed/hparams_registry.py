...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('irm_lambda', 1e2, lambda r: 10**r.uniform(-1, 2))
        _hparam('irm_penalty_anneal_iters', 600,
                lambda r: int(10**r.uniform(0, 2)))
        ...
        _hparam('mmd_gamma', 1., lambda r: 10**r.uniform(-1, 0.5))
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('lr', 1e-3, lambda r: 0.)

    if dataset in SMALL_IMAGES:
        _hparam('weight_decay', 0., lambda r: 0.)
    else:
        _hparam('weight_decay', 5e-5, lambda r: 0.)

    if dataset in SMALL_IMAGES:
        _hparam('batch_size', 42, lambda r: int(2**r.uniform(3, 9)))
    elif algorithm == 'ARM':
        _hparam('batch_size', 8, lambda r: 8)
    elif dataset == 'DomainNet':
        _hparam('batch_size', 32, lambda r: int(2**r.uniform(3, 5)))
    else:
        _hparam('batch_size', 42, lambda r: int(2**r.uniform(3, 5.5)))
